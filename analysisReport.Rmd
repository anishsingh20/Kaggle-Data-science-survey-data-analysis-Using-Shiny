...
#barplot of Gender
hchart(SurveyDf$GenderSelect,type="bar",color="green") %>%
  hc_exporting(enabled = TRUE) %>%
  hc_title(text="Barplot of gender",align="center") %>%
  hc_add_theme(hc_theme_elementary()) 

#barplot of Emp_status
hchart(SurveyDf$EmploymentStatus,type="bar",color="red") %>% 
  hc_exporting(enabled = TRUE) %>%
...
#barplot of country
hchart(SurveyDf$Country,type="bar",color="blue") %>% 
  hc_exporting(enabled = TRUE) %>%
...
hchart(countryCount,hcaes(Var1,value=Freq,color=Freq),type="treemap") %>%
  hc_exporting(enabled = TRUE) %>%
...
From the above histogram the highest count is of participants form __age 20 to 25__. This indicated that most of the freshers in the job industry,students have taken this survey and are interested in Data Science and its related domains.
...
hcboxplot(x= Age , var = EmploymentStatus,color="purple",outliers = FALSE) %>%
