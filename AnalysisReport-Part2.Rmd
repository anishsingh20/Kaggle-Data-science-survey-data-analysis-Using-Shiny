...
Now let's see which ages group of people use which tool.


```{r message=FALSE, warning=FALSE}

AgeTooldf<-SurveyDf %>% group_by(MLToolNextYearSelect) %>%
  filter(MLToolNextYearSelect %in% toptooldf$Tool) %>%
...
hcboxplot(x = SurveyDf$Age, var =SurveyDf$MLMethodNextYearSelect ,
          name = "Age", color = "#E127AB",outliers = FALSE) %>%
